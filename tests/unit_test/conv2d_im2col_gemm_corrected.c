                                         ...
                                         enum csinn_dtype_enum dtype)
{
    ...
    // CRITICAL FIX: Make a copy of kernel data and let the function reorder it
    void *kernel_copy = shl_mem_alloc(kernel_size * sizeof(float));
    memcpy(kernel_copy, kernel_data, kernel_size * sizeof(float));
    kernel->data = kernel_copy;
    
    // The function will do the reordering internally
...
int main(int argc, char **argv)
{
    ...
    // CRITICAL FIX: Use original kernel (conv2d_im2col_fp32_ker) instead of pre-reordered (conv2d_im2col_fp32_ker1)
    verify_conv2d_im2col_compute(conv2d_im2col_fp32_in, conv2d_im2col_fp32_ker,  // <-- Changed from ker1 to ker
    ...
    // CRITICAL FIX: Use original kernel (conv2d_im2col_fp16_ker) instead of pre-reordered (conv2d_im2col_fp16_ker1)
    verify_conv2d_im2col_compute(conv2d_im2col_fp16_in, conv2d_im2col_fp16_ker,  // <-- Changed from ker1 to ker
